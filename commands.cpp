...
void helpCommand(const vector<string>& argsList) {
	if (argsList.size() < 2) {
		...
		cout << BWHT "Commands:" reset "\n"
			 ...
			 << BCYN "valgrind " BCYN "(v)" reset " - compile the given file and run valgrind\n"
			 << BCYN "gcov " BCYN "(v)" reset "     - generate code coverage for the given test suite file" << endl;
		...
		if (helpCmd == "help" || helpCmd == "h") {
			...
			cout << BWHT "Usage: " GRN << argsList[0] << CYN " [compile | c] " BWHT "[<file> | <file>.cpp] [options]" reset "\n"
				 ...
				 << "\t" CYN "--watch " BCYN "(-w)" reset "		      - watch the source files for changes (" BWHT "quit" reset " or " BWHT "q" reset
					" to stop)\n"
			...
			cout << BWHT "Usage: " GRN << argsList[0] << CYN " [valgrind | v] " BWHT "[<file> | <file>.cpp] [options]" reset "\n"
				 ...
				 << "\t" CYN "--ignore-mold " BCYN "(-m)" reset "            - don't use mold as the linker\n"
				 << "\t" CYN "--ignore-lld " BCYN "(-l)" reset "             - don't use ld.lld (llvm linker) as the linker" << endl;
		} else if (helpCmd == "gcov" || helpCmd == "g") {
			cout << BWHT "Usage: " GRN << argsList[0] << CYN " [gcov | g] " BWHT "[<file> | <file>.cpp] [options]" reset "\n"
				 << "Generates code coverage for the given test suite file.\n\n"
				 << BWHT "Options:" reset "\n"
				 << "\t" CYN "--output " BCYN "(-o) " BWHT "<name>" reset "          - output file name (default same name as source)\n"
				 << "\t" CYN "--folder " BCYN "(-f) " BWHT "<folder>" reset "        - folder to output to (default \"bin\")\n"
				 << "\t" CYN "--args " BCYN "(-a) " BWHT "<arguments>" reset "       - arguments to pass to the executable\n"
...
void gcovCommand(const string& file, const map<Flag, string>& args, const SystemRequirements& sys) {
	...
	if (args.count(WATCH_FLAG)) {
		...
		auto initialCompile = [&args, &mainFile, &outputFolder, &sys, &file](const SourceSet& sources) {
			try {
				for (const Implementation& source : sources.sources) {
					compileToObject(source.path, args, sys, false, true);
				}

				string executablePath = compileObjects(mainFile, args, sys, false, true), runCmd = executablePath;
				...
				for (const Implementation& source : sources.sources) {
					if (source.path != sources.main.path) {
						allFiles += getParentDirectory(source.path) + "/" + stripExtension(stripDirectories(executablePath)) + "-" +
									stripDirectories(source.path) + " ";
					}
				}

				system(("gcov -mr -o " + outputFolder + " " + stripWhitespace(allFiles)).c_str());
				system(("rm " + outputFolder + "/*.gcda " + outputFolder + "/*.gcno").c_str());
			} catch (exception& e) {
				cout << "Compilation " BHRED "failed..." reset "\n" << e.what() << endl;
			}
		};
		auto onChange = [&args, &mainFile, &outputFolder, &sys, &file](const SourceDiff& diff, const string& changedFile, const SourceSet& sources) {
			try {
				compileToObject(changedFile, args, sys, false, true);
				...
				for (const string& source : diff.added) {
					compileToObject(source, args, sys, false, true);
				}
		...
		string executablePath = directCompile(sources, args, sys, false, true), runCmd = executablePath;
		...
		for (const Implementation& source : sources.sources) {
			if (source.path != sources.main.path) {
				allFiles +=
					getParentDirectory(source.path) + "/" + stripExtension(stripDirectories(executablePath)) + "-" + stripDirectories(source.path) + " ";
			}
		}
